...
async function main() {
  ...
  const dataLight = await getData(`${baseRequestURL}/v1/data/DropGroup_Light`);

  const itemData = {};

  const duoItems = [205504, 205506, 205409];

  const soloItems = [205503, 205408];
        ...
        ...(itemProbabilities[group] || []),
        {
          ...
          probability:
            item.dropType === "Random"
              ? Number((item.probability / totalProbability) * 100).toFixed(2)
              : item.dropType === "Probability" ? item.probability : "100",
        },
      ];
    });
  }
